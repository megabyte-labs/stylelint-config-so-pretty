...
This project and everyone participating in it is governed by the [Code of Conduct](https://github.com/megabyte-labs/stylelint-config-so-pretty/blob/master/docs/CODE_OF_CONDUCT.md). By participating, you are expected to uphold this code. Please report unacceptable behavior to [help@megabyte.space](mailto:help@megabyte.space).
...
> stylelint-config-so-pretty@1.0.4 info
> npm-scripts-info
...
All pull requests should be associated with issues. You can find the [issues board on GitLab](https://gitlab.com/megabyte-labs/docker/stylelint-config-so-pretty/-/issues). The pull requests should be made to [the GitLab repository](https://gitlab.com/megabyte-labs/docker/stylelint-config-so-pretty) instead of the [GitHub repository](ProfessorManhattan/npm-stylelint-config-so-pretty). This is because we use GitLab as our primary repository and mirror the changes to GitHub for the community.
